...
export default function SlotDataTable({ dataSet }) {
    ...
    const vaccineType = [
        ...
        { name: 'SPUTNIK V', value: 'SPUTNIK V' }
    ];
    ...
    const header = ( <
        ...
        <
        Button icon = { `pi ${isPlayState === "play" ? 'pi-volume-up' : 'pi-volume-off'}` }
        className = "p-mr-2"
        onClick = {
            () => setIsPlayState(isPlayState === "play" ? "pause" : "play") }
        /> <
    ...
    const doseFilter = < MultiSelect value = { selectedDose }
    options = { doses }
    ...
    const feeTypeFilter = < MultiSelect value = { selectedFeeType }
    options = { feeType }
    ...
    placeholder = "All"
    className = "p-column-filter" / > ;
    const ageLimitFilter = < MultiSelect value = { selectedAgeLimit }
    options = { ageLimit }
    ...
    placeholder = "All"
    className = "p-column-filter" / > ;
    const vaccineTypeFilter = < MultiSelect value = { selectedVaccineType }
    options = { vaccineType }
    onChange = { onVaccineTypeChange }
    optionLabel = "name"
    optionValue = "value"
    placeholder = "All"
    className = "p-column-filter" / > ;
    ...
    return ( <
        ...
        emptyMessage = { < h2 style = {
            ...
            field = "name"
            header = "Name"
            body = { BodyTemplates.nameBodyTemplate }
            sortable / >
            <
            Column style = {
                { width: '10%' } }
            field = "pincode"
            header = "Pincode"
            body = { BodyTemplates.pincodeBodyTemplate }
            sortable / >
            <
            Column field = "date"
            header = "Date"
            body = { BodyTemplates.dateBodyTemplate }
            filter filterElement = { dateFilter }
            filterFunction = { filterDate }
            /> <
            Column field = "dose"
            header = "Dose (No of Slots)"
            body = { BodyTemplates.doseBodyTemplate }
            filter filterElement = { doseFilter }
            /> <
            Column field = "fee_type"
            header = "Fee (Price)"
            body = { BodyTemplates.feeTypeBodyTemplate }
            filter filterElement = { feeTypeFilter }
            /> <
            Column field = "age"
            header = "Age"
            body = { BodyTemplates.ageLimitBodyTemplate }
            filter filterElement = { ageLimitFilter }
            /> <
            Column field = "vaccine"
            header = "Vaccine"
            body = { BodyTemplates.vaccineTypeBodyTemplate }
            filter filterElement = { vaccineTypeFilter }
            /> <
            Column field = "null"
            body = { BodyTemplates.bookingBodyTemplate }
            /> <
            /DataTable> <
            /div> <
            audio ref = { audioRef } >
